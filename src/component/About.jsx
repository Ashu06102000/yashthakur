  ...
  return (
    <section id="about" className="relative flex flex-col min-h-screen max-w-screen-lg m-auto">
      ...
      <div className="flex-1 px-6 md:px-14 pt-14 pb-10">
        ...
        {/* Heading */}
        <h2 className="font-sans font-thin text-[clamp(28px,5vw,48px)] leading-[1.1] text-black underline decoration-[1.5px] underline-offset-4 max-w-4xl md:text-left">
        ...
        <div className="flex items-center md:items-start flex-col gap-3 mt-10 ml-0 md:ml-2">
          <img
            src="/arrow.svg"
            alt="arrow"
            className="w-8 h-8 opacity-60 md:rotate-0 rotate-90"
          />
          <p className="font-sans text-xs text-black/50 leading-snug max-w-[220px] text-center md:text-left">
            I don’t just build interfaces<br className="hidden md:block" />
            - I build systems that last.
          </p>
        </div>

        {/* Two-column — cat on left, copy on right */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-20 gap-y-12 mt-12 md:mt-6 items-center md:items-start">
          ...
          {/* Bullet copy */}
          <div className="flex flex-col gap-6 justify-center md:pt-10">
